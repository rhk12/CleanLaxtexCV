...
def extract_2013_courses(text):
    # Course descriptions and excluded courses
    course_descriptions = {
    ...
    # Find the start and end of the 2013 course listings section
    start_of_2013 = text.find(r'\subsubsection{2013}\label{section-10}')
    end_of_2013 = text.find(r'\subsection{Service}\label{service}')

    # Extract the 2013 course listings section
    course_section_2013 = text[start_of_2013:end_of_2013]

    # Extract course numbers for 2013
    course_nums_2013 = re.findall(r'(?:ME|M\s?E) (\d{3})', course_section_2013)
    course_nums_2013 = set(course_nums_2013)  # remove duplicates

    # Convert course numbers to descriptions
    courses_2013 = []
    for num in course_nums_2013:
        if num not in excluded_courses:
            if num in course_descriptions:
                courses_2013.append(course_descriptions[num])
            else:
                courses_2013.append(f"ME{num}")

    # Print the courses for debugging
    print(f"Courses for 2013: {courses_2013}")
...
def process_courses(text):
    ...
    end_of_section = text.find(r'\subsubsection{2013}\label{section-10}') + len(r'\subsubsection{2013}\label{section-10}')
    ...
    for year, courses in grouped_sections:
        if year == "2013":
            courses_2013 = extract_2013_courses(text)
            courses_to_write = list(courses_2013)
        else:
